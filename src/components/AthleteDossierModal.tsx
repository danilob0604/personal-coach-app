...
const AthleteDossierModalContent: React.FC<{ athlete: Athlete; onClose: () => void }> = ({ athlete, onClose }) => {
  ...
  return (
    <div 
      className="fixed inset-0 z-50 flex items-center justify-center p-2 sm:p-4 bg-slate-900/60 backdrop-blur-md animate-in fade-in"
    ...
    >
      <div className="bg-white border border-slate-200 rounded-2xl sm:rounded-3xl max-w-3xl w-full max-h-[94vh] sm:max-h-[92vh] overflow-y-auto shadow-2xl text-slate-900 flex flex-col">
        ...
        <div className="p-3.5 sm:p-5 border-b border-slate-200 flex items-start justify-between bg-slate-50/80 gap-2 shrink-0">
        ...
        <div className="flex items-center gap-1.5 sm:gap-2 px-3 sm:px-5 pt-2 sm:pt-3 border-b border-slate-200 bg-white overflow-x-auto text-xs font-mono shrink-0 [scrollbar-width:none] [-ms-overflow-style:none] [&::-webkit-scrollbar]:hidden">
          <button
            type="button"
            onClick={() => setActiveTab('anagrafica')}
            className={`flex items-center gap-1.5 px-3 py-2.5 border-b-2 font-bold uppercase transition-all cursor-pointer whitespace-nowrap text-[11px] sm:text-xs touch-manipulation ${
          ...
          <button
            type="button"
            id="coach-dossier-tab-diario"
            onClick={() => {
              console.log('AGENT_CLICKED_DIARIO_ACTIVATING');
              setActiveTab('diario');
            }}
            className={`flex items-center gap-1.5 px-3 py-2.5 border-b-2 font-bold uppercase transition-all cursor-pointer whitespace-nowrap text-[11px] sm:text-xs touch-manipulation ${
              activeTab === 'diario'
                ? 'border-emerald-600 text-emerald-600 font-black bg-emerald-50/70 rounded-t-lg'
                : 'border-transparent text-emerald-700 hover:text-emerald-900 bg-emerald-50/50 rounded-t-lg font-bold'
            }`}
          >
          ...
          <button
            ...
            className={`flex items-center gap-1.5 px-3 py-2 border-b-2 font-bold uppercase transition-all cursor-pointer whitespace-nowrap text-[11px] sm:text-xs ${
              activeTab === 'recupero'
                ? 'border-blue-600 text-blue-600 font-black'
                : 'border-transparent text-slate-500 hover:text-slate-900'
            }`}
                ...
                <div>
                  ...
                  <div className="flex gap-2">
                    ...
                    <button
                      type="button"
                      onClick={() => {
                        const cleanPhone = (formData.phone || '+39 340 9876543').replace(/[^0-9]/g, '');
                        const firstName = formData.name.split(' ')[0];
                        const text = `Ciao ${firstName}! 👋 Sono il tuo Coach. Come vanno gli allenamenti? Ti aspetto per la prossima sessione! 💪`;
                        window.open(`https://wa.me/${cleanPhone}?text=${encodeURIComponent(text)}`, '_blank');
                      }}
                      className="px-3 py-2 rounded-xl bg-emerald-600 hover:bg-emerald-700 text-white text-xs font-mono font-black uppercase flex items-center gap-1.5 shadow-xs transition-colors cursor-pointer shrink-0"
                      title="Apri chat WhatsApp con l'atleta"
                    >
