...
export const AthleteApp: React.FC = () => {
  ...
  return (
    <div className="w-full max-w-4xl mx-auto px-1 sm:px-4 py-1.5 sm:py-4 space-y-2 sm:space-y-3.5">
      {/* Floating Rest Countdown Alert if timer active and athlete leaves workout view */}
      {restTimer.active && restTimer.remaining > 0 && !inWorkoutSession && (() => {
        const minutes = Math.floor(restTimer.remaining / 60);
        const seconds = restTimer.remaining % 60;
        const formattedTime = `${minutes.toString().padStart(2, '0')}:${seconds.toString().padStart(2, '0')}`;
        return (
          <div className="bg-slate-950 border-2 border-amber-400 rounded-xl sm:rounded-2xl p-2.5 sm:p-3.5 text-white flex items-center justify-between shadow-xl animate-in slide-in-from-top-2">
            <div className="flex items-center gap-2">
              <span className="w-2 h-2 rounded-full bg-amber-400 animate-ping" />
              <div>
                <div className="text-[9px] sm:text-[10px] font-mono text-amber-400 font-bold uppercase tracking-wider">
                  Recupero • {restTimer.exerciseName}
                </div>
            ...
            <div className="flex items-center gap-1.5">
              <button
                ...
                className="px-3 py-1.5 rounded-lg sm:rounded-xl bg-gradient-to-r from-blue-600 to-indigo-600 text-white font-mono text-xs font-bold uppercase tracking-wider cursor-pointer shadow-md shadow-blue-500/20"
              >
                Riprendi ➔
              </button>
              <button
        ...
        <>
          ...
          {!inWorkoutSession ? (
            ...
            <div className="space-y-2 sm:space-y-3.5">
              ...
              {/* TECHNOGYM LIVE REST COUNTDOWN HUD (COMPACT & HIGH IMPACT) */}
              {restTimer.active && restTimer.remaining > 0 && (() => {
                ...
                return (
                  <div className={`rounded-xl sm:rounded-2xl p-2.5 sm:p-3 transition-all duration-300 shadow-lg border-2 animate-in slide-in-from-top-2 ${
                    isUrgent
                      ? 'bg-gradient-to-r from-amber-950 via-slate-900 to-amber-950 border-amber-400 animate-pulse shadow-amber-500/20'
                      : 'bg-slate-950 border-amber-400/90 shadow-slate-900/40'
                    ...
                    <div className="flex items-center justify-between mb-1">
                      <div className="flex items-center gap-1.5 min-w-0">
                        <span className={`w-2 h-2 rounded-full shrink-0 ${isUrgent ? 'bg-rose-500 animate-ping' : 'bg-amber-400 animate-ping'}`} />
                        <span className="text-[9px] sm:text-[10px] font-mono font-black uppercase tracking-widest text-amber-400 truncate">
                      ...
                      <div className="flex items-center gap-1.5 shrink-0">
                        <span className="text-[9px] font-mono text-slate-400">
                          {t.athlete.coachLabel} <strong className="text-white font-mono">{total}s</strong>
                        </span>
                        <button
                          onClick={dismissRestTimer}
                    ...
                    <div className="w-full bg-slate-800/80 h-1.5 rounded-full overflow-hidden border border-slate-700/60 my-1.5">
                      <div
                    ...
                    <div className="flex items-center gap-1.5 pt-0.5">
                      <button
                        ...
                        className="px-2.5 py-1 rounded-lg bg-slate-800 hover:bg-slate-700 text-slate-200 border border-slate-700 text-[11px] font-mono font-black uppercase flex items-center justify-center gap-0.5 transition-all cursor-pointer active:scale-95"
                      ...
                      <button
                        onClick={dismissRestTimer}
                        className="flex-1 py-1 px-2.5 rounded-lg bg-gradient-to-r from-emerald-500 to-teal-500 hover:from-emerald-600 hover:to-teal-600 text-white text-xs font-mono font-black uppercase tracking-wider flex items-center justify-center gap-1 shadow-xs transition-all cursor-pointer active:scale-95"
                      >
